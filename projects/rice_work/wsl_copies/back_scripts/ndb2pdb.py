...
**note this is not used in cndb2pdb.py**
"""
...
parser.add_argument('-n', action='store', default='chromatin', dest='arg_name_R', help='Name of output file')
...
# Define subtype-to-residue mappings
# Expand or modify this dictionary based on known subtypes in your data
subtype_to_residue = {
    'A1': 'GLY', 'A2': 'GLY',
    'B1': 'GLY', 'B2': 'GLY', 'B3': 'GLY', 'B4': 'GLY',
    'NA': 'GLY', 'UN': 'GLY',
    # Add additional subtype mappings here if needed
}
...
name     = arguments.arg_name_R
pdbf = open(name + '.pdb', "w+")

print('Converting file...')

title_options = ['HEADER', 'OBSLTE', 'TITLE ', 'SPLT  ', 'CAVEAT', 'COMPND', 'SOURCE',
                 'KEYWDS', 'EXPDTA', 'NUMMDL', 'MDLTYP', 'AUTHOR', 'REVDAT', 'SPRSDE', 'JRNL  ', 'REMARK']
...
for line in file_ndb:
    ...
    if entry in title_options:
        ...
        subtype = line[16:18].strip()
        if subtype not in subtype_to_residue:
            print(f"Warning: Unknown subtype '{subtype}' encountered. Defaulting to 'GLY'.")
        residue = subtype_to_residue.get(subtype, 'GLY')
        pdbf.write(atom.format(
            int(line[7:15]), 'CA', '', residue, '', int(line[31:39]), '',
            float(line[40:48]), float(line[49:57]), float(line[58:66]),
            1.00, 0.00, 'C', '') + '\n')
        atom_serial = int(line[7:15])
        res_sequence = int(line[31:39])
    elif 'TER   ' == entry:
        pdbf.write(ter.format(atom_serial, residue, '', res_sequence, '') + '\n')
...
# Chrom_types_NDB = ['A1' for i in range(5000)] # ['A1','A2','B1','B2','B3','B4','NA'] # changed UN to NA
# Res_types_PDB   = ['GLY' for i in range(5000)]
