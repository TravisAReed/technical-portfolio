...
for mdl in range(1, totalModel-1, 1):
    ...
    for i, line in zip(list(range(len(data))), data):
        my_elements = [
            item.decode("utf-8") if isinstance(item, bytes) else item for item in line
        ]
        ndbf.write(
            ndb_string.format(
                "CHROM ",
                i + 1,
                type_list[i].decode('utf-8'),
                " ",
                "C1",
                i + 1,
                my_elements[0],
    ...
    ndbf.write(ter_string.format("TER", len(data) + 1, type_list[-1].decode('utf-8'), "C1"))
    ndbf.write("\nENDMDL\n")
    model += 1
