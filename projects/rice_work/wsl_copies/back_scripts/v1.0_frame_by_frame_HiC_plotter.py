print('Make sure all trajectories are stored in the format: chain0sim{sim}.cndb')
...
def loadTrajs(sims, trajFolder):
    ...
    for sim in sims: #made dict so sim number used as key without needing to subtract 1
        trajs[sim] = [loadTraj(f'{trajFolder}/chain0sim{sim}.cndb')]
...
parser.add_argument("--mu", type=float, required=False, default=3.22)
parser.add_argument("--rc", type=float, required=False, default=1.78)

args = parser.parse_args()

trajFolder = input('Trajectory folder (where the cndb trajectries are stored): ')
simSelectionInp = input('Sims used (ex 1,2,3 for using sims 1,2, and 3 Return no text for using all sims): ')
...
doParallelInp = input('Run HiC map plotting in parallel? (y/n): ').upper() #generates plots much faster but is still in development so it may crash while generating a lot of plots.
...
def overSimsXYZ(frame, sims):
    ...
    def xyzSub(traj, frame, beadSelection=None, XYZ=[0,1,2]): #check if works for ndb also
        R""" 
        Get the selected beads' 3D position from a **cndb** for a frame.
    ...
    sim_list = []
    for sim in sims:       
        coords0 = xyzSub(traj=trajs[sim][0], frame=frame, XYZ=[0, 1, 2]) #using loadTrajs
        sim_list.append(coords0)
...
def plotHiCs(sims, frameSelection, doDiffs=False, doParallel=False, mu=3.22, rc=1.78):
    ...
    if doDiffs: #defining initHiC #plotting with matrix subtraction (red and blue)
        initHiC = init_HiC(endFrames=args.endFrames, initTraj=args.initTraj)
